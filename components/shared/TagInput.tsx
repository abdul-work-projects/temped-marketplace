...
export default function TagInput({ value, onChange, availableTags, placeholder = 'Select tags...', loading }: TagInputProps) {
  ...
  return (
    <div ref={containerRef} className="relative">
      ...
      >
        ...
        {value.length === 0 && (
          <span className="text-sm text-muted-foreground py-1">{loading ? 'Loading tags...' : placeholder}</span>
        )}
        <ChevronDown size={16} className={`ml-auto self-center text-muted-foreground transition-transform ${open ? 'rotate-180' : ''}`} />
      </div>
