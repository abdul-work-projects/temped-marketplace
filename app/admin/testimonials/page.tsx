...
function StatusBadge({ status }: { status: string }) {
  ...
  return (
    <Badge variant="outline" className={styles[status] || 'bg-muted text-muted-foreground border-border'}>
...
function ProfileHoverLink({ profile }: { profile: TestimonialProfileInfo }) {
  ...
  return (
    ...
    >
      {profile.profileUrl ? (
        <Link href={profile.profileUrl} className="text-primary hover:underline font-medium">
          {profile.name}
        </Link>
      ) : (
        ...
        >
          <div className="flex items-center gap-3 mb-3">
            ...
            <div className="min-w-0">
              <p className="font-bold text-foreground text-sm truncate">{profile.name}</p>
          ...
          <div className="space-y-2 text-xs text-muted-foreground">
            ...
            {profile.educationPhases && profile.educationPhases.length > 0 && (
              <div className="flex items-start gap-1.5">
                <GraduationCap size={12} className="text-muted-foreground flex-shrink-0 mt-0.5" />
                <div className="flex flex-wrap gap-1">
                  {profile.educationPhases.map(p => (
                    <span key={p} className="px-1.5 py-0.5 bg-muted text-muted-foreground rounded text-[10px] font-medium">{p}</span>
                  ))}
                </div>
              </div>
            )}
              ...
              <div className="flex items-start gap-1.5">
                ...
                <div className="flex flex-wrap gap-1">
                  {profile.subjects.map(s => (
                    <span key={s} className="px-1.5 py-0.5 bg-primary/5 text-primary rounded text-[10px] font-medium">{s}</span>
                  ))}
                </div>
              </div>
            )}
