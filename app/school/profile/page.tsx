...
import { Building2, MapPin, FileText, Loader2, MessageSquare, Pencil, BadgeCheck, Clock, Shield, ShieldAlert } from 'lucide-react';
            ...
            <div className="flex-1 min-w-0">
              <div className="flex items-center gap-3 mb-1 flex-wrap">
                ...
                {(!school.verificationStatus || school.verificationStatus === 'unverified') && (
                  <Badge className="bg-muted text-muted-foreground border-border"><ShieldAlert size={14} />Unverified</Badge>
                )}
              </div>
                  ...
                  <div className="inline-block">
                    {isCertImage ? (
                      <img
                        ...
                        onClick={() => setLightbox({ src: certUrl, alt: 'Registration Certificate' })}
                      />
                    ) : isCertPdf ? (
                      <button
                        onClick={() => setLightbox({ src: certUrl, alt: 'Registration Certificate', fileName: 'certificate.pdf' })}
              ...
              <div className="space-y-3">
                <div className="flex items-center gap-2">
                  <div className={`w-3 h-3 rounded-full ${
                    school.verificationStatus === 'approved' ? 'bg-green-500' :
                    school.verificationStatus === 'pending' ? 'bg-yellow-500' :
                    school.verificationStatus === 'rejected' ? 'bg-red-500' :
                    'bg-gray-300'
                  }`}></div>
                  <span className="text-sm text-muted-foreground">Registration Certificate</span>
                </div>
                {school.verificationStatus === 'rejected' && school.rejectionReason && (
                  <div className="ml-5 p-3 bg-red-50 border border-red-200 rounded-lg">
                    <p className="text-sm text-red-700">
                      <span className="font-medium">Rejected:</span> {school.rejectionReason}
                    </p>
                    <Link href="/school/setup" className="text-sm text-red-600 font-medium underline mt-1 inline-block">
                ...
                {(!school.verificationStatus || school.verificationStatus === 'unverified') && (
                  <div className="ml-5">
                    <p className="text-sm text-muted-foreground">
                      ...
                      <Link href="/school/setup" className="text-primary font-medium underline">
                        Go to setup
                      </Link>
                    </p>
                  </div>
                ...
                {school.verificationStatus === 'pending' && (
                  <div className="ml-5">
                    <p className="text-sm text-muted-foreground">Your certificate is being reviewed by an admin.</p>
                  </div>
                )}
              </div>
            </div>
